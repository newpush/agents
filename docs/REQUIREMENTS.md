...
- **Specialized Components**: The "Support Helper" persona and "WHMCS MCP" protocol documentation are planned for a future release and will be maintained in a separate specialized repository; this core library focuses on general-purpose agents.
...
2. **Configuration**: Agent runtime execution is configured via the orchestrator's environment variables. The context assembly script uses `mcp.config.json` as the source of truth for context assembly (generating `GEMINI.md`).
...
4. **Resilience & Logging**: Agents must handle tool execution and API failures gracefully. A standardized "Error Handling and Resilience" directive is maintained in global guidelines. Standardized logging to `stdout` and `stderr` is the responsibility of the execution platform or orchestrator, not the agent personas.
...
- **Data Persistence**: The core execution model is stateless execution. The `pgvector` memory layer (found in Docker examples) is an optional enhancement for advanced orchestrator setups and is not a core requirement.
- **Runtime Environment**:
    - **Language**: The toolkit primarily utilizes a Node.js environment for context generation scripts. Python runtime support is officially deprecated; the repository is a language-agnostic library of specifications.
    - **Dependencies**: Docker and Gemini CLI are required dependencies for running local examples and pre-flight checks, but not for reading the library definitions.
