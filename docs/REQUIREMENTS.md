...
- The Fleet Dashboard reference implementation (`examples/gatekeeper-deployment/dashboard-ingest.js`) accepts both the canonical `/api/v1/reports` path and the legacy `/ingest` path; the `docker-compose.yml` agent now points at the canonical path (Decision [2026-04-29]). Legacy compatibility may be removed in a future cleanup.
...
- The `Client Onboarding` validation workflow now references machine-readable starter test vectors at `examples/red-team-gauntlet/test-vectors.yaml` (Decision [2026-04-29]); additional categories beyond the 5 starter cases remain open for expansion.
...
- **Audit Script Gaps (partial — JSON schema only)**: `scripts/audit-repo.js` now enforces structural contracts (Required Headings, Refusal Criteria) on both `agents/` and `skills/`, with case-insensitive heading matching (Decision [2026-04-29]). It still does not perform JSON schema validation for the `Audit Log` content — that remains an open clarification.
...
- **Reference Example Completeness (partial)**: `examples/red-team-gauntlet/` now ships a starter `test-vectors.yaml` with 5 cases (Decision [2026-04-29]); other reference directories may still need additional assets.
...
- ~~**Sync Script Hardcoding**~~ Resolved (Decision [2026-04-29]): `scripts/sync-upstream.sh` now reads its remote, URL, branch, and organization label from `NOEMI_SYNC_*` environment variables; the documentation explains the contract.
- ~~**Audit Script Heading Case Sensitivity**~~ Resolved (Decision [2026-04-29]): `scripts/audit-repo.js` now performs case-insensitive heading matching for both agents and skills.
