...
- **Node.js 24 Baseline Drift**: Resolved (Decision [2026-05-08]) — `examples/gatekeeper-deployment/docker-compose.yml` and `tools/executive-assistant/Dockerfile` now pin `node:24-alpine`. Remaining Node-based runtimes should follow the same baseline.
